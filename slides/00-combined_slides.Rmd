...
- Four scales you can set with `option = `: `"plasma"`, `"magma"`, `"inferno"` and `"viridis"` (the default)
...
# Other common options: greyscale
...
- But we can make our own with `scale_colour_gradient`
...
- `scale_colour_manual()`
- Other colours [complementary](https://www.sessions.edu/color-calculator-results/?colors=009dc6,c69e00,c6004f) to Understanding Inequalities blue
- See more in the [`ggplot2` references](https://ggplot2.tidyverse.org/reference/scale_manual.html#examples)
...
# hex codes have to b in a call to c()
...
- IDK why this is but I'm sure there are good reasons for it!
...
- One of `gganimate`'s stock examples:
]
...
- Thomas Lin Pedersen's [`ggplot2` workshop materials]( https://github.com/thomasp85/ggplot2_workshop)
...
.pull-left[
- Hopefully we've convinced you that `ggplot2` is __powerful__ and __flexible__
- It might be confusing at first, but the best way to get comfortable with it is just to play around!
- Go forth and plot
]

.pull-right[

```{r hadley_plot, out.height = '90%', echo = FALSE, fig.align = "center"}
