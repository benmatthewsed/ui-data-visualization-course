...
# `Your turn!`
# Lines __xxx__ in the __exercises (add link to these here)__
# Lines __xxx__ in the __answers (add link to these here)__
...
.pull-right[
```{r ref.label = 'layers_2', echo = FALSE, fig.retina = 3}
```
]
...
- It's kind of a lie to have ``geom_text` is a funny one because you __can__ use it to add values from your dataset using `aes()` (in this case, the values from the `y` axis):
...
- From the [`ggplot2` book](https://ggplot2-book.org/getting-started.html#qplot-facetting): "Faceting creates __tables of graphics__ by splitting the data into subsets and displaying the __same graph for each subset__"
...
- `ggplot` then makes a long string of plots - one for each level of the faceting variable - which it __wraps__ into a table
...
- The `ggplot` book has a [whole section](https://ggplot2-book.org/facet.html#facet-wrap) on faceting
...
- __[Match perceptual and data topology](http://stat405.had.co.nz/lectures/20-effective-vis.pdf)__: If you’re using a graphical element to show relative size of quantities __make larger size = larger quantity__
    - Does a brighter colour imply higher or lower value?
...
```{r my_graph, out.width = '50%', echo = FALSE}
...
- __Symptom__: ["Sawtooth"](https://ggplot2-book.org/collective-geoms.html) lines on your plot.

- From __R for Data Science__:
...
- __Symptom__: Error message `geom_path: Each group consists of only one observation. Do you need to adjust the group aesthetic?`

- __Diagnosis__: This happens when you try to plot a __line__ using a factor as one of the position scales (usually `x` in my experience). `ggplot` then 'guesses' that you want the `x` variable to be the grouping variable

- Thing to note - data types matter to `ggplot`!
...
2. Groups == only one observation (categorical/factor x-variable)
    - Colouring by a factor variable or a continuous one
- Data types matter to `ggplot2`
- If you want to plot a line you need to make sure the x and y axes are __continuous__ variables and that you're not trying to colour

---
# `stat_count() must not be used with a y aesthetic`

- __Symptom__: Error message `stat_count() must not be used with a y aesthetic`
...
- __Prescription__: You can tell `geom_bar` that you've summarised the data already by specifying `geom_bar(stat = "identity")` (here `identity` just means "use the numbers that I gave you please and don't calculate anything with them")
...
- __Symptom__: Error message
- __Diagnosis__: `ggplot` is looking for something in your dataset but there are no variables with that name
- See our discussion on concepts in the grammar of graphics about what goes inside the call to `aes` and what goes outside
- __Prescription__: Make sure there aren't any typos, and that what you've specified is in your dataset
- If you're trying to e.g. set a colour you might have to move this outside the call to `aes()`
...
# Dataset + 
# mapping variables in the dataset to `aes`thetic + 
# Layers of `geom`s (symbols to represent data) = 
# Plot
