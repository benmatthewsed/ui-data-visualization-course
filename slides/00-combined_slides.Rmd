...
# Maps in `R` are ["different and weird"](https://djnavarro.net/post/maps/)
...
- Using `sf`, the geographical information in your dataset lives in a special `geometry` column in your data frame
- This is __works nicely with tidy data__ and `ggplot2`!
- `ggplot2` figures out how to draw the map based on the CRS, so this changes some of how we would usually ask `ggplot2` to draw the map. We'll walk through an example of this in the exercises.
...
- This is a bit involved, but something to bear in mind if you're mapping spatial data with variable sample sizes in different regions:
...
# Intro to `sf` and what's in my spatial dataframe?

---

# Mapping with simple features data
- __Simple features__ can be thought of as "things" or objects that have a __spatial__ location or extent
- The `{sf}` package in `R` allows us to implement simple features
...
- The nice thing about working with `sf` data is that each row of data corresponds to a distinct geographical unit
